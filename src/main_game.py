...
def play_turn(connexion: sqlalchemy.Connection,
              board: Board) -> Dict[str, Union[Type[Player], str]]:
    """
    ...
    if game.is_stalemate(player_color):
        ...
        game.set_finish()
        print("Equality between the player", game.get_joueur(0),
    ...
    while True:
        ...
        while True:
            ...
            location_piece_to_be_moved = input(
                "Enter the destination square (example: a4): ").lower()

            if len(location_piece_to_be_moved) < 2:
                print("Invalid input! Must be a letter followed by a number.")
                continue

            if not (location_piece_to_be_moved[0].isalpha() and
                    location_piece_to_be_moved[1:].isdigit()):
                print("Invalid input! Must be a letter followed by a number.")
                continue
